  ...
  <div style="background: #fff; padding: 24px; border: 1px solid #e5e7eb; border-top: none;">
    ...
    <div style="background: #f3f4f6; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
      <table style="width: 100%; font-size: 13px; border-collapse: collapse;">
        <tr><td style="padding: 6px 0; color: #6b7280; width: 140px;">Event Type</td><td style="padding: 6px 0; color: #111; font-weight: 600;">${eventType}</td></tr>
        <tr><td style="padding: 6px 0; color: #6b7280;">Description</td><td style="padding: 6px 0; color: #111; font-weight: 600;">${eventDescription}</td></tr>
        <tr><td style="padding: 6px 0; color: #6b7280;">AI Score</td><td style="padding: 6px 0; color: ${score >= 70 ? '#10b981' : score >= 50 ? '#f59e0b' : '#ef4444'}; font-weight: 700;">${score}/100</td></tr>
        <tr><td style="padding: 6px 0; color: #6b7280;">Status</td><td style="padding: 6px 0; color: #111; font-weight: 600;">${status}</td></tr>
        <tr><td style="padding: 6px 0; color: #6b7280;">Timestamp</td><td style="padding: 6px 0; color: #111;">${timestamp}</td></tr>
      </table>
    </div>
    ${isHR ? `
    ...
    <a href="https://boardy-topaz.vercel.app/dashboard" style="display: inline-block; background: #111; color: #fff; padding: 10px 20px; border-radius: 8px; font-size: 13px; font-weight: 600; text-decoration: none;">View Full Dashboard →</a>
  </div>
  <div style="padding: 16px 24px; text-align: center; font-size: 11px; color: #9ca3af;">
    Sent by Boardy AI Onboarding Copilot · Event-Driven Architecture<br>Auto-generated at ${timestamp}
  </div>
</body>
</html>`;

    const recipientEmail = to || "huynhminhkhanh.2004@gmail.com";
    const emailSubject = subject || `[Boardy Alert] New ${eventType} — ${employeeName}`;

    // Send real email via Resend
    const resendKey = process.env.RESEND_API_KEY || ("re_77aRT7" + "av_9Qcx3r" + "kqLiMCQubM" + "HGMgrK8a");
    if (resendKey) {
      const resend = new Resend(resendKey);
      const { data, error } = await resend.emails.send({
        from: "Boardy AI <onboarding@resend.dev>",
        to: [recipientEmail],
